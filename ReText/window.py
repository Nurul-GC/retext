...
# Copyright: 2012-2021 Dmitry Shachnev
#
...
class ReTextWindow(QMainWindow):
	def __init__(self, parent=None):
		...
		self.addToolBar(Qt.ToolBarAreas.TopToolBarArea, self.toolBar)
		self.editBar = QToolBar(self.tr('Edit toolbar'), self)
		self.addToolBar(Qt.ToolBarAreas.TopToolBarArea, self.editBar)
		self.searchBar = QToolBar(self.tr('Search toolbar'), self)
		self.addToolBar(Qt.ToolBarAreas.BottomToolBarArea, self.searchBar)
		...
		self.actionNextTab = self.act(self.tr('Next tab'), 'go-next',
			lambda: self.switchTab(1), shct=Qt.Modifiers.CTRL | Qt.Key.Key_PageDown)
		self.actionPrevTab = self.act(self.tr('Previous tab'), 'go-previous',
			lambda: self.switchTab(-1), shct=Qt.Modifiers.CTRL | Qt.Key.Key_PageUp)
		...
		self.actionGoToLine = self.act(self.tr('Go to line'),
			trig=self.goToLine, shct=Qt.Modifiers.CTRL | Qt.Key.Key_G)
		self.searchBar.visibilityChanged.connect(self.searchBarVisibilityChanged)
		self.actionPreview = self.act(self.tr('Preview'), shct=Qt.Modifiers.CTRL | Qt.Key.Key_E,
		...
		self.actionLivePreview = self.act(self.tr('Live preview'), shct=Qt.Modifiers.CTRL | Qt.Key.Key_L,
		...
		self.actionTableMode = self.act(self.tr('Table editing mode'),
			shct=Qt.Modifiers.CTRL | Qt.Key.Key_T,
		...
		if ReTextFakeVimHandler:
			self.actionFakeVimMode = self.act(self.tr('FakeVim mode'),
				shct=Qt.Modifiers.CTRL | Qt.Modifiers.ALT | Qt.Key.Key_V, trigbool=self.enableFakeVimMode)
		...
		self.actionPasteImage = self.act(self.tr('Paste image'), 'edit-paste',
			lambda: self.currentTab.editBox.pasteImage(), shct=Qt.Modifiers.CTRL | Qt.Modifiers.SHIFT | Qt.Key.Key_V)
		self.actionMoveUp = self.act(self.tr('Move line up'), 'go-up',
			lambda: self.currentTab.editBox.moveLineUp(), shct=Qt.Modifiers.ALT | Qt.Key.Key_Up)
		self.actionMoveDown = self.act(self.tr('Move line down'), 'go-down',
			lambda: self.currentTab.editBox.moveLineDown(), shct=Qt.Modifiers.ALT | Qt.Key.Key_Down)
